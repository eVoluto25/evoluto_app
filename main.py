...
async def chat(payload: dict):
    ...
    try:
        response = f"Risposta generica a: {message}"
...
@app.post("/upload_pdf")
async def upload_pdf(file: UploadFile = File(...)):
    logging.info(f"PDF ricevuto: {file.filename}")
    try:
        # Simulazione URL GPT + Claude
        gpt_url = f"https://mock.supabase.io/gpt/{file.filename}.html"
        claude_url = f"https://mock.supabase.io/claude/{file.filename}.html"
